/**
 * QR Code Generator (纯前端离线生成)
 * 基于 Kazuhiko Arase 的 qrcode-generator 算法 (public domain)
 * 适配微信小程序 Canvas 2D API
 */

// ===== Galois Field GF(256) 运算 =====
var QRMath = (function () {
...
function QR8bitByte(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    this.parsedData = this.parsedData.concat(byteData);
  ...
  this.parsedData = Array.prototype.slice.call(new Uint8Array(this.parsedData));
}
...
/**
 * 根据文本内容自动选择最小可用版本（1-40）
 * @param {string} text 待编码文本
 * @param {string} level 纠错级别 'L'|'M'|'Q'|'H'
 * @returns {number} typeNumber
 */
function detectTypeNumber(text, level) {
  var ecl = QRErrorCorrectLevel[level] !== undefined ? QRErrorCorrectLevel[level] : QRErrorCorrectLevel.M;
  for (var type = 1; type <= 40; type++) {
    ...
    var totalDataCount = 0;
    for (var i = 0; i < rsBlocks.length; i++) totalDataCount += rsBlocks[i].dataCount;
    if (buffer.getLengthInBits() <= totalDataCount * 8) return type;
  }
  return 0;
}

/**
 * 生成二维码并返回模块矩阵 (boolean[][])
 * @param {string} text 编码内容
 * @param {string} level 'L'|'M'|'Q'|'H'，默认 'M'
 * @returns {boolean[][]} 二维码模块矩阵
 */
function createMatrix(text, level) {
  var ecl = QRErrorCorrectLevel[level] !== undefined ? QRErrorCorrectLevel[level] : QRErrorCorrectLevel.M;
  var typeNumber = detectTypeNumber(text, level || 'M');
  if (!typeNumber) throw new Error('内容过长，无法生成二维码');
